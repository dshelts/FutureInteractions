...
class SampleListener(Leap.Listener):
	def on_init(self, controller):
		...
		self.oportal = Oportal(oportal_image, (image_width, image_height), (WIDTH//2, 0), SIZE)
	...
	def on_frame(self, controller):
		...
		self.screen.blit(self.oportal.image, self.oportal.move())

		if self.oportal.surrounds((scaledX, scaledY)):
			old_x, old_y = self.last_pos

			self.oportal.vY = 0
			self.oportal.vX = 0

			self.oportal.x = scaledX-(self.ball.width//2)
			self.oportal.y = scaledY-(self.ball.height//2)
...
class oportal():
	def __init__(self, image, size=(50, 50), pos=(300, 0), bounds=(350, 350)):
		self.image = image
