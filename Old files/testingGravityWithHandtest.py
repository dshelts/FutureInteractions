...
class SampleListener(Leap.Listener):
	...
	def on_frame(self, controller):
		...
		#background = load_background_image(tyson.jpg)
...
class Ball():
	...
	def teleport(self):
		...
		if (self.x == portals.x1) and (self.y == portals.y1):
			self.x = portals.x2
			self.y = portals.y2 -40

		elif(self.x == portals.x2) and (self.y == portals.y2):
			self.x = portals.x1
			self.y = portals.y1 +40
